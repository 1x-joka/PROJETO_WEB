...
if (isset($_POST['acao'])) {
    if ($_POST['acao'] === 'cadastrar') {
        $nome = $_POST['nome']; $partido = $_POST['partido_politico']; $nascimento = $_POST['data_nascimento'];
        $idade = $_POST['idade']; $inicio_mandato = $_POST['data_inicio_mandato'];
        ...
        $stmt->bindParam(':nome', $nome); $stmt->bindParam(':partido', $partido); $stmt->bindParam(':nascimento', $nascimento);
        $stmt->bindParam(':idade', $idade); $stmt->bindParam(':inicio_mandato', $inicio_mandato);
        $stmt->bindParam(':final_mandato', $final_mandato); $stmt->bindParam(':id_pais', $id_pais); $stmt->bindParam(':id_cidade', $id_cidade);
    ...
    if ($_POST['acao'] === 'excluir' && isset($_POST['id_governante'])) {
        $id = $_POST['id_governante'];
        $sql = "DELETE FROM governante WHERE id_governante = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
        echo "<script>alert('Governante excluído!'); window.location.href='../index.php';</script>";
    }
}
?>